...
import { fetchYoutubeChannelAnalytics } from "./youtube";
...
const server = createServer(async (req: IncomingMessage, res: ServerResponse) => {
  ...
  try {
    ...
    if (url.pathname === "/api/youtube/channel") {
      ...
      if (!channelId && !handle && !lookup) {
        sendJSON(res, 400, { error: "Missing lookup" }, origin);
        return;
      }
      ...
      sendJSON(res, 200, analytics, origin);
      return;
    }
